...
export function Comparison({ t }: { t: Dict; lang: 'en' | 'ko' }) {
  ...
  return (
    <section className="mx-auto max-w-4xl px-6 py-16">
      <p className="text-sm font-medium text-indigo-600 dark:text-indigo-400">{c.eyebrow}</p>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="border-b border-black/15 dark:border-white/15">
              ...
              <th className="py-3 text-indigo-600 dark:text-indigo-400 font-semibold">{c.headChronos}</th>
            </tr>
          </thead>
          <tbody>
            {c.rows.map((r) => (
              <tr key={r.label} className="border-b border-black/5 dark:border-white/5">
                ...
                <td className="py-3 text-center text-indigo-600 dark:text-indigo-400">{r.chronos}</td>
              </tr>
            ))}
          </tbody>
        </table>
